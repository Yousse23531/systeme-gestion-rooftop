...
export const getEmployees = (): Employee[] => {
  // Synchronous wrapper reading from localStorage cache; if empty, try sql.js asynchronously by caller if needed
...
export const getRecettes = (): Recette[] => {
  const data = localStorage.getItem(STORAGE_KEYS.RECETTES);
  return data ? JSON.parse(data) : [];
};
...
export const getMonthlyArchives = (): MonthlyArchive[] => {
  const data = localStorage.getItem(STORAGE_KEYS.MONTHLY_ARCHIVES);
  return data ? JSON.parse(data) : [];
};
...
export const getHistoriqueAchats = (): Achat[] => {
  const data = localStorage.getItem(STORAGE_KEYS.HISTORIQUE_ACHATS);
  return data ? JSON.parse(data) : [];
};
...
export const getHistoriqueMaintenances = (): Maintenance[] => {
  const data = localStorage.getItem(STORAGE_KEYS.HISTORIQUE_MAINTENANCES);
  return data ? JSON.parse(data) : [];
};
...
export const getHistoriqueRecettes = (): Recette[] => {
  const data = localStorage.getItem(STORAGE_KEYS.HISTORIQUE_RECETTES);
  return data ? JSON.parse(data) : [];
};
...
export const getDashboardHistory = (): DashboardHistory[] => {
  const data = localStorage.getItem(STORAGE_KEYS.DASHBOARD_HISTORY);
  return data ? JSON.parse(data) : [];
};
